...
def send_user_data_to_db(name, dob, qualification, phone_number, email, course_ordered):
    ...
    try:
        ...
        print(f"✅ User successfully added with User ID: {user_data['id']}")
        ...
        order_payload = {
            "user_id": user_data['id'],  # Link the order to the newly created user
            "course_name": course_ordered
        }
        ...
        print(f"✅ Order successfully added with Order ID: {order_data['id']}")
...
def Poshbot():
    ...
    while True:
        ...
        if user_input.lower() in ["i am done", "exit", "quit"]:
            print(
                "Bot 🤖: Thanks for choosing Poshem Technologies Institution. This is your course ID. You'll get a mail from us shortly.")
            break
